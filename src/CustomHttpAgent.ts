...
export class CustomHttpAgent extends HttpAgent {
    ...
    run(input: RunAgentInput): Observable<BaseEvent> {
        ...
        // @ag-ui/client ships its own bundled copy of rxjs; the Observable type
        // it exports is structurally identical but nominally different from the
        // rxjs we import here, so TS treats them as incompatible. The library's
        // vite.config.lib.ts aliases `rxjs` to the project root to dedupe the
        // two copies at build time, but the cast remains as a belt-and-braces
        // defense for consumers whose bundler doesn't deduplicate the same way.
